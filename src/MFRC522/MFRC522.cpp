...
MFRC522::StatusCode MFRC522::PCD_CalculateCRC(
    ...
    while (esp_timer_get_time() - timestamp < 100000) {
...
void MFRC522::PCD_Init() {
    ...
    PCD_WriteRegister(TPrescalerReg, 0xA9);  // TPreScaler = TModeReg[3..0]:TPrescalerReg, ie 0x0A9 = 169 =>
                                             // f_timer=40kHz, ie a timer period of 25μs.
    PCD_WriteRegister(TReloadRegH, 0x03);    // Reload timer with 0x3E8 = 1000, ie 25ms before timeout.
    PCD_WriteRegister(TReloadRegL, 0xE8);

    PCD_WriteRegister(
        TxASKReg, 0x40);  // Default 0x00. Force a 100 % ASK modulation independent of the ModGsPReg register setting
...
MFRC522::StatusCode MFRC522::PCD_CommunicateWithPICC(
    ...
    while (!(isTimeout = esp_timer_get_time() - timestamp >= 50000)) {
    ...
    // 50ms and nothing happend. Communication with the MFRC522 might be down.
    if (isTimeout) {
        return STATUS_TIMEOUT;
    }
    ...
    if (backData && backLen) {
        ...
        *backLen = n;                                         // Number of bytes returned
        ...
        _validBits =
            PCD_ReadRegister(ControlReg) & 0x07;  // RxLastBits[2:0] indicates the number of valid bits in the last
...
MFRC522::StatusCode MFRC522::PICC_Select(
    ...
    // UID): High nibble: complete
    // bytes, Low nibble: Extra bits. 		Byte 2: UID-data or CT		See explanation below. CT means Cascade
    // Tag. 		Byte 3: UID-data 		Byte 4: UID-data 		Byte 5: UID-data
