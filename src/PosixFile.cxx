...
File PosixFile::create(FILE* file, const std::string& name) { return File(std::make_shared<PosixFile>(file, name)); }

File PosixFile::open(const char* name, const char* mode) {
    return File(std::make_shared<PosixFile>(fopen(name, mode), name));
}
